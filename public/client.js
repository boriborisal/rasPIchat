...
// 재연결 중 플래그 — join-room이 서버에서 처리(room-users 수신)될 때까지 send-file 지연
// 카메라 촬영 후 복귀 시 send-file이 join-room보다 먼저 도달하는 race condition 방지
let _rejoining   = false;
let _pendingFile = null; // 재연결 중 전송 대기 파일
...
socket.on('room-users', ({ count, users, hostNickname }) => {
  ...
  // 재연결 완료 — join-room이 서버에서 처리됨. 대기 중인 파일 전송
  if (_rejoining) {
    _rejoining = false;
    if (_pendingFile) {
...
function sendFile(file) {
  ...
  // 재연결 중이거나 소켓 미연결 — room-users 수신(join-room 처리 완료) 후 자동 재시도
  // 카메라 촬영 후 복귀 시 발생하는 send-file/join-room race condition 방지
  if (_rejoining || !socket.connected) {
    _pendingFile = file;
    return;
  }
...
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'visible') {
    if (!socket.connected) {
      // 연결 끊김 → 재연결 (onReconnect에서 _rejoining=true + join-room 재전송)
      socket.connect();
    } else {
      // 연결 살아있지만 iOS 등에서 이벤트 누락 가능 → join-room 재전송으로 이력 동기화
      // room-history 핸들러가 existingCount 이후분만 추가하므로 중복 없음
      // _rejoining=true: join-room 처리 완료(room-users 수신) 전까지 send-file 지연
      _rejoining = true;
      socket.emit('join-room', { code: roomCode, nickname, isHost });
